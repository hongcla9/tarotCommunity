...
function TarotBoard() {
  ...
  useEffect(() => {
    const fetchArray = query => {
      ...
      axios.post(`/api/tboard/gettboard?page=${page}`)
        .then(response => {
  ...
  const makePagination = () => (
    [...Array(Pages)].map((x, page) => 
      <span class={ (CurrentPage * 1) === (page + 1) ? 'page active' : 'page' } onClick={event => setCurrentPage(event.target.innerText)}>{page + 1}</span>
    )
  )
